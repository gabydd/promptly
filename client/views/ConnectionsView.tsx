...
import { CompleteUserProps } from "../lib/types";
import { CONNECT } from "../lib/queries";



export const ConnectionsView = ({ refetchUser, user }: CompleteUserProps) => {
  const [addConnection, { data }] = useMutation(CONNECT, {
  ...
  return (
    <div className="bg-origin-padding rounded-lg bg-gray-300 bg-opacity-90 shadow-2xl mt-10 ml-10 p-4">
        ...
        >
          <Form>
            ...
            <div className="text-red-600 text-sm mt-2">
              {data ? data.addConnection.message : undefined}
            </div>
          </Form>
        </Formik>
      </div>
      ...
      {user.connections!.map((connection) => {
        return (
          <div key={connection.name} className="text-yellow-700">
